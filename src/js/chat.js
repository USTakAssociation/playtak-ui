// Must use 'var' because inline HTML onclick handlers access this via window.chathandler
var chathandler={
	chat_width: 200,
	rooms: {},
	cur_room: 'global-',
	...
	selectRoom: function(id){
		if(this.rooms.hasOwnProperty(id)){
			if(this.rooms.hasOwnProperty(this.cur_room)){
				const oldroom=this.rooms[this.cur_room];
	...
	$.contextMenu({
		...
		items: {
			...
			Games: {
				name: "Past Games",
				callback: function(key,opt){
					const name = opt.$trigger[0].innerText.split(':')[0].replace(/[^a-zA-Z0-9_]/g,"");
					//yuck.. but we don't need any more sophistication
					const url="https://www.playtak.com/games/search?player_white="+name+"&mirror=true";
					window.open(url);
				}
			}
		}
